---
id: business.logistics
title: "Логистика"
last_updated: "2026-04-16"
updated_by: "claude"
tags: [logistics, delivery, pickup, route, vehicle, business]
related: [business, business.order, business.reception]
---

# Логистика

Логистика — опциональный независимый модуль. Система работает без него (клиент может самостоятельно привезти и забрать изделия). Модуль не знает о бизнес-логике оснований — он универсален.

## Роли

| Роль | Доступ к системе | Ответственность |
|------|:---:|----------------|
| **Водитель** | ✗ | Управляет транспортом |
| **Экспедитор** | ✓ | Сопровождает груз, отмечает забор и доставку в системе |

Физически водитель и экспедитор могут быть одним человеком.

## Сущности

### Транспортное средство

| Атрибут | Описание |
|---------|----------|
| Название / номер | Идентификатор |
| Грузоподъёмность | Объём в кг — учитывается при планировании рейса |

### Задача логистики

Минимальная единица — одно перемещение от точки А до точки Б.

| Атрибут | Описание |
|---------|----------|
| Основание | Любой документ: заказ, приёмка, закупка и т.д. |
| Откуда | Адрес отправления |
| Куда | Адрес назначения |
| Статус | Новая → Назначена → В пути → Выполнена |

Отображаемый подстатус формируется из основания и направления:

| Основание | Направление | Подстатус для заказа |
|-----------|------------|---------------------|
| Заказ | клиент → производство | «в пути на производство» |
| Приёмка | производство → клиент | «доставляется клиенту» |
| Закупка | поставщик → производство | (не влияет на заказ) |

### Маршрутный лист

Объединяет несколько задач логистики в один рейс.

| Атрибут | Описание |
|---------|----------|
| Транспортное средство | Машина с грузоподъёмностью |
| Водитель | Кто ведёт |
| Экспедитор | Кто сопровождает и работает в системе |
| Задачи | Упорядоченный список остановок |
| Статус | Новый → В рейсе → Завершён |

## Структура рейса

За один рейс экспедитор может объехать несколько адресов и несколько раз заехать на производство:

```mermaid
flowchart LR
    START([Выезд]) --> A1[Адрес клиента А\nзабор]
    A1 --> B1[Производство\nзавоз]
    B1 --> A2[Адрес клиента Б\nзабор]
    A2 --> A3[Адрес клиента В\nдоставка]
    A3 --> B2[Производство\nзабор готового]
    B2 --> A4[Адрес клиента Г\nдоставка]
    A4 --> END([Конец рейса])
```

## Жизненный цикл задачи логистики

```mermaid
stateDiagram-v2
    [*] --> Новая : Создана (менеджером или автоматически)
    Новая --> Назначена : Добавлена в маршрутный лист
    Назначена --> В_пути : Маршрутный лист начат
    В_пути --> Выполнена : Экспедитор отметил
    Назначена --> Новая : Убрана из маршрутного листа
    Выполнена --> [*]
```

## Грузоподъёмность

При формировании маршрутного листа учитывается суммарный вес задач относительно грузоподъёмности транспортного средства. Это информационный контроль — не жёсткая блокировка.
